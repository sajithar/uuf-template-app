...
(function () {
    ...
    /**
     * Whether the specified div is processable or not.
     * @param div {UIComponent} div to be checked
     * @param user {User} current user
     * @return {boolean} <code>true</code> if processable, otherwise <code>false</code>
     */
    function isDivProcessable(div, user) {
        if (div.disabled) {
        ...
        if (utils.parseBoolean(divDefinition[constants.UI_COMPONENT_DEFINITION_IS_ANONYMOUS])) {
            // This is an anonymous div. So no need for an user session or checking permissions.
            return true;
        }

        // This is not an anonymous div.
        if (user) {
            // An user has logged in.
            var divPermissions = divDefinition[constants.UI_COMPONENT_DEFINITION_PERMISSIONS];
            if (divPermissions && Array.isArray(divPermissions)) {
                // A permissions array is specified in the div definition.
                var numberOfDivPermissions = divPermissions.length;
                var userPermissionsMap = user.permissions;
                for (var i = 0; i < numberOfDivPermissions; i++) {
                    if (!userPermissionsMap.hasOwnProperty(divPermissions[i])) {
                        if (log.isDebugEnabled()) {
                            log.debug("User '" + user.username + "' in domain '" + user.domain
                                      + "' does not have permission '" + divPermissions[i]
                                      + "' to view div '" + div.fullName + "'.");
                        }
                        return false;
                    }
                }
                ...
                // Permissions are not specified in the div definition.
                return true;
            }
        } else {
    ...
    function pageHelper(mentionedPageFullName, handlebarsOptions) {
        ...
        var pagePublicUri = appContext + "/" + constants.DIRECTORY_APP_DIV_PUBLIC + "/"
        ...
        if (handlebarsOptions.fn) {
            // {{#page "pageName"}} {{#zone "_pushedDivs"}} ... {{/zone}} {{/page}}
    ...
    /**
     * 'div' Handlebars helper function.
     * @param mentionedDivFullName {string}
     * @param handlebarsOptions {Object}
     * @return {SafeString}
     */
    function divHelper(mentionedDivFullName, handlebarsOptions) {
        var mentionedDiv = lookupTable.divs[mentionedDivFullName];
        if (!mentionedDiv) {
            var msg = "Div '" + mentionedDivFullName + "' does not exists.";
            log.error(msg);
            throw new Error(msg);
        }

        var processingDiv = getFurthestChild(mentionedDiv);
        var currentUser = renderingContext.user;
        if (!isDivProcessable(processingDiv, currentUser)) {
            return new handlebarsEnvironment.SafeString("");
        }

        if (log.isDebugEnabled() && (mentionedDiv.fullName != processingDiv.fullName)) {
            log.debug("Div '" + processingDiv.fullName + "' is processed for div '"
                      + mentionedDiv.fullName + "'.");
        }
        ...
        // Start processing div 'processingDiv'.
        uiComponentsStack.push(processingDiv);
        ...
        var optionsHash = handlebarsOptions.hash;
        var optionsHashParams = optionsHash[constants.HELPER_PARAM_PARAMS];
        var divParams = (optionsHashParams) ? optionsHashParams : optionsHash;
        var divPublicUri = appContext + "/" + constants.DIRECTORY_APP_DIV_PUBLIC + "/"
                            + processingDiv.fullName;
        var uriParams = renderingContext.uriParams;
        var user = renderingContext.user;
        var scriptContext = {
            app: {name: appName, context: appContext, conf: appConf},
            div: {params: divParams, publicUri: divPublicUri},
            uriParams: uriParams,
            user: user,
            handlebars: handlebarsEnvironment
        };
        var templateContext = executeScript(processingDiv, scriptContext);
        // Additional parameters to the template context.
        var templateOptions = {
            data: {
                app: {name: appName, context: appContext, conf: appConf},
        ...
        var returningHtml = "";
        // Process this div's template.
        var processingDivTemplateFilePath = processingDiv.templateFilePath;
        if (processingDivTemplateFilePath) {
            var divContent = readFile(processingDivTemplateFilePath);
            if (!divContent) {
                var msg = "Cannot read template '" + processingDivTemplateFilePath + "' of div '"
                          + processingDiv.fullName + "'.";
                log.error(msg);
                throw new Error(msg);
            }
            var divHtml = compileTemplate(divContent)(templateContext, templateOptions).trim();
            if (divHtml.length > 0) {
                returningHtml = divHtml;
            }
        }
        // Process parents' templates from nearest to furthest.
        var parentDivs = processingDiv.parents;
        var numberOfParentDivs = parentDivs.length;
        for (var i = 0; i < numberOfParentDivs; i++) {
            var parentDiv = parentDivs[i];
            var parentDivTemplateFilePath = parentDiv.templateFilePath;
            if (parentDivTemplateFilePath) {
                var parentDivContent = readFile(parentDivTemplateFilePath);
                if (!parentDivContent) {
                    var msg = "Cannot read template '" + parentDivTemplateFilePath + "' of div '"
                              + parentDiv.fullName + "'.";
                    log.error(msg);
                    throw new Error(msg);
                }
                var parentDivCompiledTemplate = compileTemplate(parentDivContent);
                var parentDivHtml = parentDivCompiledTemplate(templateContext,
                                                                templateOptions).trim();
                if ((returningHtml.length == 0) && (parentDivHtml.length > 0)) {
                    // Child div haven't given any "returning" HTML.
                    returningHtml = parentDivHtml;
                }
            }
        }
        uiComponentsStack.pop();
        // Finished processing div 'processingDiv'.
    ...
    function resourceHelper(type, path, handlebarsOptions) {
        ...
        if (!resourceProvider) {
            ...
            throw new Error("'" + type
                            + "' Handlebars helper should be used inside a page or an div.");
        }
            ...
            if (mainZone) {
                ...
                if (mainZone.hasResources()) {
                    var publicUri = renderingContext.app.context + "/"
                                    + constants.DIRECTORY_APP_DIV_PUBLIC + "/";
                    var resourcesBuffer = [];
                ...
                var isInProtectedScope, childDivFullName;
                if (handlebarsOptions.hash[constants.HELPER_PARAM_SCOPE] == "protected") {
                    ...
                    var uiComponentsStackSize = uiComponentsStack.length;
                    if (uiComponentsStackSize > 0) {
                        // Now we are inside an div. Here 'scope' parameter can be used with
                        // 'defineZone' helper for main-zones.
                        isInProtectedScope = true;
                        // When processing a parent-div, last element of the divs stack contains
                        // its child-div.
                        childDivFullName = uiComponentsStack[uiComponentsStackSize - 1].fullName;
                    } else {
                        // Not inside an div. So 'scope' parameter has no effect.
                        isInProtectedScope = false;
                        childDivFullName = null;
                    }
                }
                ...
                for (var i = 0; i < numberOfContentProviders; i++) {
                    ...
                    if (isInProtectedScope
                        && (childDivFullName != contentProviderFullName)) {
                        // Scope of this main-zone is 'protected'. So only contents provided by
                        // child divs should be processed. Content provider 'contentProviders[i]'
                        // is not a child-div of the processing parent-div.
                        continue;
                    }
